...
import {updateBackground,  backgroundColor} from "../utils/getBackgroundColor";

const props = defineProps({
  albumInfo: {}
});
const gradientColor = computed(() => `linear-gradient(to bottom, ${backgroundColor.value} , #1F1F1F 50%)`)
const albumInfo = ref({ id: 1, image: require('../assets/pictures/playlists/1.jpg'), name: '歌单 1', creator: 'Kriskyy', songCount: 6, totalTime: '20:54' });
const musicList = ref([
  {
    id: 1,
    number: 1,
...
const handleResize = ()=>{
  ...
  if(window.innerWidth < 605)
  {
    ...
    albumText.forEach(album => {album.style.visibility = "hidden";});

  }
  else{
    albums.forEach(album => {album.style.visibility = "visible";});
    albumText.forEach(album => {album.style.visibility = "visible";});
  }
  ...
  <div class="album-content" :style="{backgroundImage: gradientColor}">
    <div class="header" >
      ...
      <div class="header-content">
        ...
        <p class="header-album-name" style="font-weight: bolder;font-size:100px;margin:10px 0 35px 10px;">{{ albumInfo.name }}</p>
        <div class="header-content-detail">
          <p class="header-creator"  @click="enterPersonalSpace">{{ albumInfo.creator }}</p>
          <p>•</p>
          <p style="margin-left:6px">歌曲数量: {{ albumInfo.songCount }}</p>
        </div>
      </div>
    </div>
    <div class="content">
      <div class="play-area">
        ...
        <el-popover
          ...
          <ul>
            <li @click="editAlbumDiscription">编辑歌单详情</li>
      ...
      <div class="musicList">
        ...
        >
          ...
          <div :style="{visibility: musicHoveredIndex === music.id ? 'hidden' : 'visible' }">{{ music.number }}</div>
          <play-button @click="playFromId(music.id)" style="position: absolute;left: 33px;cursor: pointer" v-if="musicHoveredIndex === music.id"/>
          ...
          <div class="music-detailed-info">
            ...
            <div class="music-name-author" style="padding-left: 5px;">
              <p @click="enterMusicDescription(music.id)" class="music-name"  >{{ music.name}}</p>
              <p @click="enterAuthorDescription(music.author)" class="music-author"  :style="{color:musicHoveredIndex === music.id? 'white' : '#b2b2b2'}">{{ music.author}}</p>
            </div>
          </div>

          <div class="music-album-info" :style="{color:musicHoveredIndex === music.id? 'white' : '#b2b2b2'}">{{ music.album }}</div>
          <div class="music-right-info">
            <check-mark @onclick="addToFavorite(music.id)" class="check-mark"  :style="{visibility: musicHoveredIndex === music.id ? 'visible' : 'hidden'}"/>
            <div style="margin-left: auto;margin-right: 15px; color: #b2b2b2" :style="{color:musicHoveredIndex === music.id? 'white' : '#b2b2b2'}">{{ music.time }}</div>
      ...
      <div class="other-info">
        ...
        <div class="music-item" v-for="music in musicList" :key="music.id + 'recommend'">
          ...
          <img :src="music.img" alt="推荐歌曲图片" class="music-image"/>
